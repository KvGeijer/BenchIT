 ...
 * $URL: svn+ssh://william@rupert.zih.tu-dresden.de/svn-base/benchit-root/BenchITv6/kernel/numerical/matmul/C/OpenMP/MKL/double/dgemm_init.c $
...
void bi_getinfo(bi_info* infostruct) {
   ...
   bi_parselist(p);
	...
	infostruct->kerneldescription = bi_strdup("Matrix Multiply, BLAS, MKL (C) - OpenMP version");	
...
void* bi_init(int problemSizemax) {
	...
	myfds=(floating_data_struct *)malloc(sizeof(fds));
	if(myfds==NULL) {
...
extern void bi_cleanup(void *mcb) {
	fds *data=(floating_data_struct *)mcb;
	free(data);
}
