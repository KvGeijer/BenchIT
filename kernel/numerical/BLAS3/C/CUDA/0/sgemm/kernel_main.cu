 ...
 * $URL: svn+ssh://benchit@rupert.zih.tu-dresden.de/svn-base/benchit-root/BenchITv6/kernel/numerical/BLAS3/C/CUDA/0/sgemm/kernel_main.c $
...
int setup(mydata_t *params, DT valueA, DT valueB)
{
    ...
    CUDA_CHECK(cudaMalloc((void**) &(params->devData[0]), params->maxmm * sizeof (DT)));
    CUDA_CHECK(cudaMalloc((void**) &(params->devData[1]), params->maxmm * sizeof (DT)));
    CUDA_CHECK(cudaMalloc((void**) &(params->devData[2]), params->maxmm * sizeof (DT)));

    CUDA_CHECK(cudaMemcpy(params->devData[0], params->hostData[0],
    				params->maxmm * sizeof (DT), cudaMemcpyHostToDevice));

    CUDA_CHECK(cudaMemcpy(params->devData[1], params->hostData[1],
    				params->maxmm * sizeof (DT), cudaMemcpyHostToDevice));
...
int teardown(mydata_t *params)
{
    ...
    CUDA_CHECK(cudaFree(params->devData[2]));
...
int run(mydata_t *params, DT alpha)
{
    ...
    dim3 gridDim(ceil((float)params->m / BLOCK_SIZE), ceil((float)params->m / BLOCK_SIZE));

    CUDA_CHECK(cudaMemcpy(params->devData[2], params->hostData[2],
    				params->maxmm * sizeof (DT), cudaMemcpyHostToDevice));


#ifdef PREC_SINGLE
    sgemm(gridDim, blockDim, params, alpha);
#endif

    CUDA_CHECK(cudaThreadSynchronize());

    CUDA_CHECK(cudaMemcpy(params->hostData[2], params->devData[2],
    				params->maxmm * sizeof (DT), cudaMemcpyDeviceToHost));
