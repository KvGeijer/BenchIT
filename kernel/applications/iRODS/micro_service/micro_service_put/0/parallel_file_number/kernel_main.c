...
void bi_getinfo(bi_info * infostruct)
{
   ...
   infostruct->kerneldescription = bi_strdup("iRods (Micro-Service put): Parallel transfer for a specific amount of files");
   ...
   sprintf(file_info,"<msiDataObjPutWithOptions> [Filesize: %d %s]",(penv->file_block_size*penv->file_block_number),penv->file_unit);
   infostruct->legendtexts[0] = bi_strdup(file_info);
   sprintf(file_info,"<msiDataObjPutWithOptions> [Filesize: %d %s][Rate]",(penv->file_block_size*penv->file_block_number),penv->file_unit);
