...
static int asm_work_cas(unsigned long long addr, unsigned long long passes, volatile mydata_t *data) __attribute__((noinline));
static int asm_work_cas(unsigned long long addr, unsigned long long passes, volatile mydata_t *data) {
    if (!passes) return 0;
...
// #define ONE_CHASE "mov (%%rbx), %%rbx;"NOP(NOPCOUNT)
    ...
    __asm__ __volatile__(
        TIMESTAMP
        "movq %%rax, %%r12;" // 'Backup' RAX, since it's used in cmpxchg
        SERIALIZE
        "jmp _pointer_chasing_loop;"
        ".align 64, 0x0;"
        "_pointer_chasing_loop:"
        ONE_CHASE
        ONE_CHASE
        ONE_CHASE
        ONE_CHASE
       ...
       case 0:          
               ...
               if (data->ENABLE_CODE_PREFETCH){
                  *((unsigned long long*)(data->cache_flush_area))=(unsigned long long)(data->cache_flush_area); //pointer to itself
                  for (j=0;j<data->NUM_USES;j++) {tmp+=asm_work_cas((unsigned long long)(data->cache_flush_area),1,data);}
               }
               //measurement
               if (!t) tmp=asm_work_cas(aligned_addr,accesses/24,data);
               else tmp=asm_work_cas(data->threaddata[t].aligned_addr,accesses/24,data);
               break;
       default: break;
     }
