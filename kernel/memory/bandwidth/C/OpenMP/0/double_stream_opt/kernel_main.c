 ...
 *
 * This file connects the work-part (work.c) to the interface (benchit.c)
 * via their headers
 *******************************************************************/
    ...
    if(threadPinning){
        ...
        if (b==0){
          ...
          exit(127);
   	    ...
   	    c =mdp->c[num];
        IDL(2,printf ("Thread %i/%i: allocated c:%i\n",
                      (int)omp_get_thread_num(),
                      (int)omp_get_num_threads(),
                      (int)(maxlength+offset)));
        if (c==0){
...
int bi_entry(void* mdpv, int problemsize, double* results)
{
  ...
  /* do the measurement */
	for (k=0;k<internal_repeats;k++)
	{
	  ...
	  do{
        time[0][k]=copy_(c,a,numberOfMovedDoubles/2,offset,repeats,localAlloc,threadPinning);
        // check whether overflow
        if (repeats>(repeats*10)) break;
        repeats=repeats*10;
	  }while(time[0][k]<minTimeForOneMeasurement);
    repeats=repeats/10;
	  time[0][k]=time[0][k]/repeats;
      IDL(3,printf ("start scale"));
	  /* use the (now higher) number of repitions for all other measurements  */
	  do{
        time[1][k]=scale_(b,c,scalar,numberOfMovedDoubles/2,offset,repeats,localAlloc,threadPinning);
        // check whether overflow
        if (repeats>(repeats*10)) break;
        repeats=repeats*10;
    ...
    repeats=repeats/10;
      ...
      IDL(3,printf ("start triad"));
	  do{
        time[3][k]=triad_(a,b,c,scalar,numberOfMovedDoubles/3,offset,repeats,localAlloc,threadPinning);
        // check whether overflow
        if (repeats>(repeats*10)) break;
        repeats=repeats*10;
...
void bi_cleanup(void* mdpv)
{
   ...
   // free c and its sub-arrays
   if (mdp->c!=0){
     for (i=0;i<omp_get_max_threads();i++){
       if (mdp->c[i]!=0)
         _aligned_free(mdp->c[i]);
     }
     free(mdp->c);
   }
   // free structure
