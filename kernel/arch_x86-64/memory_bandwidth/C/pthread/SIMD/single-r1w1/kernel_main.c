...
int METHOD=0,LAYOUT=0,LOCAL_READ=0,LOCAL_WRITE=0,USE_MODE_R=0,USE_MODE_W=0,FRST_SHARE_CPU=0;
int NUM_SHARED_CPUS=0,USE_DIRECTION=0,ALWAYS_FLUSH_CPU0=0;
...
void bi_getinfo( bi_info * infostruct )
{
   ...
   /* local bandwidth of first CPU in list and bandwidth between this and all other selected CPUs */
   ...
   for ( j = 0; j < n_of_works; j++ ){
     ...
     for (k=0;k<NUM_RESULTS;k++){
       ...
       switch ( j ){
         case 0:  // GB/s
           sprintf(buff,"bandwidth: CPU%llu - CPU%llu",cpu_bind[0],cpu_bind[k]);
...
void* bi_init( int problemsizemax )
{
   ...
   /* increase buffersize to account for alignment, offset, and param_t structure */
   BUFFERSIZE=sizeof(char)*(MAX+4*ALIGNMENT+OFFSET+2*sizeof(unsigned long long)+sizeof(param_t)+RUNS*ALIGNMENT);
   /* buffersize used for the measurement (without param_t structure)*/
   mdp->buffersize=(MAX+2*ALIGNMENT+OFFSET+2*sizeof(unsigned long long))&(0xffffffffffffffff^(2*ALIGNMENT-1));
#if 0
   printf("\nBUFFERSIZE: %llu\n",BUFFERSIZE);
   printf("buffersize: %llu\n",mdp->buffersize);
   printf("buffersize/2: %llu\n",mdp->buffersize/2);
   fflush(stdout);
#endif
   ...
   mdp->USE_MODE_R=USE_MODE_R;
   mdp->USE_MODE_W=USE_MODE_W;
   mdp->method=METHOD;
   mdp->layout=LAYOUT;
   mdp->alignment=ALIGNMENT;
   mdp->read_local=LOCAL_READ;
   mdp->write_local=LOCAL_WRITE;
   mdp->offset=OFFSET;
   mdp->runs=RUNS;
   mdp->extra_clflush=EXTRA_CLFLUSH;
   mdp->function=FUNCTION;
   mdp->burst_length=BURST_LENGTH;
   mdp->factor=FACTOR;
   mdp->init_value=VALUE;
  ...
  for (t=1;t<mdp->num_threads;t++){
    ...
    mdp->threaddata[t].USE_MODE_R=mdp->USE_MODE_R;
    mdp->threaddata[t].USE_MODE_W=mdp->USE_MODE_W;
  ...
  if ((mdp->buffer == 0)||(mdp->buffer == (void*) -1ULL)){
   ...
   for (i=0;i<=BUFFERSIZE-tmp;i+=tmp){
      *((double*)((unsigned long long)mdp->buffer+i))=(double)i;
   }
...
int inline bi_entry( void* mdpv, int problemsize, double* results )
{
  ...
  if (RANDOM) {
  _work(problemarray2[problemsize-1],(mydata_t*)mdpv,&results);
  results[0] = (double)(problemarray2[problemsize-1]);
  } else {
  _work(problemarray1[problemsize-1],(mydata_t*)mdpv,&results);
  results[0] = (double)(problemarray1[problemsize-1]);
  }

  /* copy papi_results into results */  
  #ifdef USE_PAPI
  for (k=0;k<NUM_RESULTS;k++)
  {
    for (j=0;j<papi_num_counters;j++)
    {
      results[1+(j+1)*NUM_RESULTS+k]=((mydata_t*)mdpv)->papi_results[j*NUM_RESULTS+k];
    }
  }
  #endif
  return 0;
...
void evaluate_environment(bi_info * info)
{
   ...
   p = bi_getenv( "BENCHIT_KERNEL_READ_LOCAL", 0 );
   if ( p != 0 ) LOCAL_READ = atoi( p );
   else LOCAL_READ=0;
   p = bi_getenv( "BENCHIT_KERNEL_WRITE_LOCAL", 0 );
   if ( p != 0 ) LOCAL_WRITE = atof( p );
   else LOCAL_WRITE=0;

   p = bi_getenv( "BENCHIT_KERNEL_FACTOR", 0 );
   if ( p != 0 ) FACTOR = atof( p );
   else FACTOR=1.0;
      p = bi_getenv( "BENCHIT_KERNEL_INIT_VALUE", 0 );
   if ( p != 0 ) VALUE = atoi( p );
   else VALUE=1.0;

   p=bi_getenv( "BENCHIT_KERNEL_METHOD", 0 );
   if (p==0) {errors++;sprintf(error_msg,"BENCHIT_KERNEL_METHOD not set");}
   else{
     if (0);
     else if (!strcmp(p,"copy")) METHOD=1;
     else if (!strcmp(p,"scale")) METHOD=2;
     else if (!strcmp(p,"indep")) METHOD=4;
     else {errors++;sprintf(error_msg,"invalid setting for BENCHIT_KERNEL_METHOD");}
   }  
 
   p=bi_getenv( "BENCHIT_KERNEL_LAYOUT", 0 );
   if (p==0) {errors++;sprintf(error_msg,"BENCHIT_KERNEL_LAYOUT not set");}
   else{
     if (!strcmp(p,"C")) LAYOUT=LAYOUT_CONT;
     else if (!strcmp(p,"F")) LAYOUT=LAYOUT_FIXED;
     else if (!strcmp(p,"A1")) LAYOUT=LAYOUT_ALT1;
     else if (!strcmp(p,"A2")) LAYOUT=LAYOUT_ALT2;
     else {errors++;sprintf(error_msg,"invalid setting for BENCHIT_KERNEL_LAYOUT");}
   } 

   p = bi_getenv( "BENCHIT_KERNEL_READ_STREAM_USE_MODE", 0 );
   if ( p == 0 ) USE_MODE_R=MODE_EXCLUSIVE;
   else{ 
     if (!strcmp(p,"M")) USE_MODE_R=MODE_MODIFIED;
     else if (!strcmp(p,"E")) USE_MODE_R=MODE_EXCLUSIVE;
     else if (!strcmp(p,"I")) USE_MODE_R=MODE_INVALID;
     else if (!strcmp(p,"S")) USE_MODE_R=MODE_SHARED;
     else if (!strcmp(p,"O")) USE_MODE_R=MODE_OWNED;
     else if (!strcmp(p,"F")) USE_MODE_R=MODE_FORWARD;
     else {errors++;sprintf(error_msg,"invalid setting for BENCHIT_KERNEL_READ_STREAM_USE_MODE");}
   }
   
   p = bi_getenv( "BENCHIT_KERNEL_WRITE_STREAM_USE_MODE", 0 );
   if ( p == 0 ) USE_MODE_W=MODE_MODIFIED;
   else{ 
     if (!strcmp(p,"M")) USE_MODE_W=MODE_MODIFIED;
     else if (!strcmp(p,"E")) USE_MODE_W=MODE_EXCLUSIVE;
     else if (!strcmp(p,"I")) USE_MODE_W=MODE_INVALID;
     else if (!strcmp(p,"S")) USE_MODE_W=MODE_SHARED;
     else if (!strcmp(p,"O")) USE_MODE_W=MODE_OWNED;
     else if (!strcmp(p,"F")) USE_MODE_W=MODE_FORWARD;
     else {errors++;sprintf(error_msg,"invalid setting for BENCHIT_KERNEL_WRITE_STREAM_USE_MODE");}
   }
   if ( ((USE_MODE_R==MODE_SHARED)||(USE_MODE_R==MODE_OWNED)||(USE_MODE_R==MODE_FORWARD)) || ((USE_MODE_W==MODE_SHARED)||(USE_MODE_W==MODE_OWNED)||(USE_MODE_W==MODE_FORWARD)))
   {
     ...
     if (p==0) {errors++;sprintf(error_msg,"BENCHIT_KERNEL_SHARE_CPU not set, required by selected BENCHIT_KERNEL_USE_MODE");}
       ...
       FRST_SHARE_CPU=atoi(p);
       if (CPU_ISSET(FRST_SHARE_CPU,&cpuset)) {errors++;sprintf(error_msg,"SHARE_CPU must not be used for measurement");}
       NUM_THREADS++;
       cpu_bind[NUM_THREADS-1]=FRST_SHARE_CPU;
       FRST_SHARE_CPU=NUM_THREADS-1;
     }
   }
   ...
   if (p==0) {errors++;sprintf(error_msg,"BENCHIT_KERNEL_INSTRUCTION not set");}
     ...
     if (0);
     else if (!strcmp(p,"movapd")) {
       ALIGNMENT=128;OFFSET=0;FUNCTION=0;
     }
     else if (!strcmp(p,"vmovapd")) {
       ALIGNMENT=256;OFFSET=0;FUNCTION=1;
     }
     else if (!strcmp(p,"movupd")) {
       ALIGNMENT=128;OFFSET=OFFSET%ALIGNMENT;FUNCTION=2;
     }
     else if (!strcmp(p,"vmovupd")) {
       ALIGNMENT=256;OFFSET=OFFSET%ALIGNMENT;FUNCTION=3;
     }
     else if (!strcmp(p,"mov")) {
       ALIGNMENT=64;OFFSET=OFFSET%ALIGNMENT;FUNCTION=4;
     }
     else if (!strcmp(p,"mov_clflush")) {
       ALIGNMENT=64;OFFSET=OFFSET%ALIGNMENT;FUNCTION=5;
     }
     else if (!strcmp(p,"movnti")) {
       ALIGNMENT=64;OFFSET=OFFSET%ALIGNMENT;FUNCTION=6;
     }
     else if (!strcmp(p,"movntpd")) {
       ALIGNMENT=128;OFFSET=OFFSET%ALIGNMENT;FUNCTION=7;
     }
     else if (!strcmp(p,"vmovntpd")) {
       ALIGNMENT=256;OFFSET=OFFSET%ALIGNMENT;FUNCTION=8;
     }
   ...
   if ((BURST_LENGTH==8)&&((FUNCTION==4)||(FUNCTION==5)||(FUNCTION==6)||0))
     {errors++;sprintf(error_msg,"BURST LENGTH %i not supported for %s",BURST_LENGTH, bi_getenv( "BENCHIT_KERNEL_INSTRUCTION", 0 ));}
