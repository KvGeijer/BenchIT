...
void bi_getinfo(bi_info * infostruct)
{
   ...
   infostruct->kerneldescription = bi_strdup("iRods (iput): Parallel transfer for a specific amount of files");
   ...
   sprintf(file_info,"<iput %s %s %s> [Filesize: %d %s][Rate]",penv->resource,penv->num_threads,penv->protocol,(penv->file_block_size*penv->file_block_number),penv->file_unit);
