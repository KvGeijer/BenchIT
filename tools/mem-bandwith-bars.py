...
def parse_arguments() -> Config:
    ...
    parser = argparse.ArgumentParser(
        description="Bandwidth Measurements for reads on different memory levels and cache states")
    parser.add_argument('--save-name', type=str,
                        help="A name to save the script results under. Is saved in the kernel output/mem-bandwidth-bars dir")
    parser.add_argument(
        '--cache-state',
        type=str,
        choices=['M', 'E', 'I'],
        default='E',
        help="Cache state: 'M' (Modified), 'E' (Exclusive), 'I' (Invalid)"
    )
    ...
    if args.save_name:
        save_name = Path('output/mem-bandwidth-bars') / args.save_name
        if save_name.exists():
...
def plot_bandwidth_data(df, config: Config):
    """
    Function to plot bandwidth data using Seaborn and Matplotlib.
    """
    ...
    df_melted = pd.melt(df_selected, id_vars=[
                        'Memory Level'], var_name='Thread', value_name='Bandwidth')
    ...
    # Create a barplot
    ax = sns.barplot(x='Memory Level', y='Bandwidth',
                     hue='Thread', data=df_melted, errorbar=None)

    # Customize the plot
    ax.set_title('Read Bandwidths from Different Threads')
...
def main():
    ...
    if not config.old_name:
        try:
            ...
            # Collect or generate bandwidth data based on configurations
