 ...
 * $URL: svn+ssh://william@rupert.zih.tu-dresden.de/svn-base/benchit-root/BenchITv6/kernel/numerical/cannon/F95/MPI/0/long/kernel_main.c $
...
void bi_getinfo(bi_info * infostruct) {
   ...
   for (ii = 0; ii < NUM_FUNC; ii++) {
      if (penv->output) {
         ...
         infostruct->yaxistexts[ii] = bi_strdup("iop/s");
         infostruct->selected_result[ii] = 0;
         infostruct->base_yaxis[ii] = 0;    // logarythmic axis 10^x
         switch (ii) {
            case 0:
               infostruct->legendtexts[ii] = bi_strdup("IOPS (ijk)");
               break;
            case 1:
               infostruct->legendtexts[ii] = bi_strdup("IOPS (ikj)");
               break;
            case 2:
               infostruct->legendtexts[ii] = bi_strdup("IOPS (jik)");
               break;
            case 3:
               infostruct->legendtexts[ii] = bi_strdup("IOPS (jki)");
               break;
            case 4:
               infostruct->legendtexts[ii] = bi_strdup("IOPS (kij)");
               break;
            case 5:
               infostruct->legendtexts[ii] = bi_strdup("IOPS (kji)");
               break;
         }
      }
   }
