 ...
 * $URL: svn+ssh://william@rupert.zih.tu-dresden.de/svn-base/benchit-root/BenchITv6/kernel/sort/selsort/C/0/0/simple/kernel_main.c $
...
void bi_getinfo(bi_info * infostruct) {
   ...
   infostruct->codesequence
      = bi_strdup("find smallest element -> change with first element");
   infostruct->kerneldescription
      = bi_strdup("Evaluate time to sort int/float array with selection sort");
...
int bi_entry(void * mdpv, int iproblemSize, double * dresults) {
   ...
   do {
      ...
      for(ii=0; ii<numloop[0]; ii++)
         selsorti(sortArray->intarray, imyproblemSize);
      dend[0] = bi_gettime();
   ...
   do {
      ...
      for(ii=0; ii<numloop[1]; ii++)
         selsortf(sortArray->floatarray, imyproblemSize);
      dend[1] = bi_gettime();
